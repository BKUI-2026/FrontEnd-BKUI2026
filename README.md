...
6. **Jangan bikin entry `FE-000X` buat perubahan kecil.** Fix typo, tweak styling, fix lint/type error, refactor kecil tanpa ubah behavior — commit biasa aja, gak usah dicatet di log. Entry baru cuma buat halaman/komponen baru, fitur yang pindah status integrasi, atau keputusan yang mempengaruhi struktur project (detail & contoh di `markdowns-fe/INDEX.md`).
7. **Jangan sentuh isi folder `BackEnd-BKUI2026/`.** Batas repo = batas tanggung jawab. Kebutuhan perubahan kontrak API dicatat di `markdowns-fe/integrations/backend-api-contract.md` dan dikoordinasikan langsung ke partner BE.
8. **Ikuti penamaan istilah dari PRD** (CASA, PJ Sekolah, Arah Petualangan, dst) di code/comment/nama komponen.
